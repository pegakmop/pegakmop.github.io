...
echo ""
echo "Данный скрипт представляет собой генерацию WARP конфигурации Cloudflare  и установку её на роутеры фирмы Keenetic при активной поддержке сайта, если бы не они я бы и не стал заморачиваться с данным скриптом"
logger "Данный скрипт представляет собой генерацию WARP конфигурации Cloudflare  и установку её на роутеры фирмы Keenetic при активной поддержке сайта, если бы не они я бы и не стал заморачиваться с данным скриптом"
echo ""
...
address_ip4_custom="172.16.$iface_num.2"
configure_wireguard() {
  ...
  logger "Возможно будет 1 или 2 ошибки красным гореть, не обращайте внимания на них, если у вас интернет только ip4, то это ошибки попытки добавления ip6 адресов. Если вы пользуетесь ip4 + ip6 ошибок добавления не будет вообще."
  echo ""
  sleep 3
  ndmc -c "interface $iface_name" >/dev/null 2>&1
  ndmc -c "interface $iface_name description @pegakmop-$iface_name" >/dev/null 2>&1
  ndmc -c "interface $iface_name ip address $address_ip4_custom/32" >/dev/null 2>&1
  ...
  echo ""
  echo "P.S. если генерируете более одного конфига и один из них не подключается, нажми на конфиг и после удали конфиг"
  logger "P.S. если генерируете более одного конфига и один из них не подключается, нажми на конфиг и после удали конфиг"
}

#echo "Используется источник: warp-gen.vercel.app"
