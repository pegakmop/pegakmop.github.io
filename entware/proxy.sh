...
ndmc -c "components list" | awk '
  ...
  found && /installed: true/ {print; exit}
' && {
  ...
  echo "➡️ В веб-интерфейсе Keenetic: http://$(ip -4 -o addr show br0 2>/dev/null | awk '{print $4}' | cut -d/ -f1 | head -n1)"
  echo "В боковом меню ищем → Параметры системы → Изменить набор компонентов → Клиент прокси → включите галочку и сохраните."
  echo "❗️Роутер перезагрузится после сохранения. Потом запустите данный скрипт заново."
  exit 1
}