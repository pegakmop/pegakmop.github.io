...
echo ""
log "Любое копирование, изменение или распространение данного скрипта в любых других ресурсах без письменного разрешения автора @pegakmop строго запрещается, использование только с данного репозитория, приятной настройки и использования sing-box-go без заморочек с автонастройкой в один клик, скрипт написан для роутеров keenetic, ориентировался по более старой и доброй архитектуре mips, думаю на других проблем не должно возникнуть, отдельно касаясь ютуберов, я против обозреванич пока не будет в публичном доступе, на данный момент это альфа тест, как будет публичный надпись исчезнет со скрипта"
echo
