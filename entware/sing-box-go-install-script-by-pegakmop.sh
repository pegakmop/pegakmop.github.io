...
echo ""
log "Любое копирование, изменение, публикация или распространение данного скрипта в любых других ресурсах без письменного разрешения автора @pegakmop (https://t.me/pegakmop) строго запрещено, будут страйки на каналы, жалобы и т.д, использование только с данного репозитория, приятной настройки и использования sing-box-go без заморочек с автонастройкой в один клик, скрипт написан для роутеров keenetic, ориентировался по более старой и доброй архитектуре mips, думаю на других проблем не должно возникнуть, отдельно касаясь ютуберов, я против обозреванич пока не будет в публичном доступе, на данный момент это альфа тест, как будет публичный надпись исчезнет со скрипта"
echo
ndmc -c "no interface Proxy0"
ndmc -c 'system configuration save'
echo "не обращайте внимания на эту ошибку, перестраховка на случай повторного запуска скрипта и переустановки сделано, что бы удалить прокси и заново поставить потом в конце установки"
...
echo
echo "⚠️По всем вопросам скрипта установщика пишем: https://t.me/pegakmop или в чате ссылка в посте: https://t.me/vpnconfiguration/62"
echo
if [ ! -f "$INPUT_FILE" ]; then
  log "⚠️ Файл $INPUT_FILE не найден. приготовьте ключ vless:// или ss:// для ввода..."
  #начало конфига
  echo "Введите ss:// или vless:// ссылку:"
  read LINK
  if [ -z "$LINK" ]; then
    echo "Ссылка не введена, выхожу."
      exit 1
  fi
...
EOF
  ...
  *)
  ...
  ndmc -c "interface Proxy0 description @pegakmop-192.168.1.1:1080"
